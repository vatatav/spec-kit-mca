...
# The file for the /analyze command is 'user_prompts/mc07-analyze-user.md'.
...
- If `.specify/memory/mc11-state.json` exists, you may read these keys to avoid re-deriving known defaults:
  - `packaging_entrypoint` — use when packaging is referenced; do not re‑inspect scripts if unchanged.
  - `hygiene_scope` — prefer this scope when referencing character hygiene checks (e.g., `src`).
- Treat hints as advisory only; do not modify files based on them. Proceed normally if the file is missing.
...
# Before any other analysis, enforce provenance presence on generated docs to preserve workflow order.
# 1) Run `.specify/scripts/powershell/check-prerequisites.ps1 -Json -PathsOnly` and parse:
#    - FEATURE_SPEC, IMPL_PLAN, TASKS
# 2) For each existing file among these, verify the first line matches:
#    `<!-- Generated-by: <command> | Timestamp: <ISO8601> -->`
# 3) If any required file is missing a header, STOP and output a clear failure with a fix hint, e.g.:
#    - "Missing provenance header in <path>. Add via .specify/scripts/provenance.ps1 -Path <path> -Generator '/plan|/tasks|manual'"
# Continue with the rest of analysis only if all present files pass the header check.
